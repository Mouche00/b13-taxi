<x-layout class="bg-[url('{{ asset('/images/parchment.jpg') }}')] bg-contain md:bg-cover bg-norepeat">
    ...
    <div class="flex flex-col lg:flex-row md:mx-12 lg:mr-12">
        <x-dashboard.sidebar role="passenger" />

        <main class="relative {{ $currentReservation ? 'w-full' : 'lg:w-[60%] mb-12 md:mb-0' }}">
                    ...
                    @if (request(['departure']) && !$drivers->isEmpty())
                        <div class="m-2 mt-8 w-fit">
                            <div class="bg-white border-black border-4 border-dashed rounded-xl font-black text-xl">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2" style="/*! grid-template-columns: 1fr max-content; */">
                                        ...
                                        <form action="/reservation/add" method="POST" class="relative flex justify-center hover:bg-black m-4 rounded-lg hover:text-white transition ease-in-out delay-150 border-2 border-black border-dashed p-4">
                                            ...
                                            <div class="flex items-center gap-2">
                                                ...
                                                <div class="grid grid-cols-3 items-center p-2  h-full">
                                                    ...
                                                    <div class="text-[10px] md:text-xs col-span-2 mx-2 text-center flex">
            ...
            @else
                <div class=" border-4 border-dashed border-black rounded-lg flex items-center mb-32 bg-white flex-col">
                    <h1 class="font-black text-white text-xl bg-black p-4 text-center w-full">Current Reservation</h1>
                    <div class="p-4 w-full">
                        <div class="relative w-full flex border-2 border-black border-dashed w-full rounded-lg flex-col md:flex-row pb-32 md:pb-0">
                            @if(!$currentReservation->favorited && $favorites->map(fn($item) => $item->route)->map(fn($item) => $item->route)->where('departure', $currentReservation->route->route->departure)->where('destination', $currentReservation->route->route->destination)->isEmpty())
                                <div class="relative flex text-white md:h-[400px] lg:h-64 w-32 items-center justify-between border-r-2 border-dashed border-black bg-[url('http://127.0.0.1:8003/images/parchment.jpg')] bg-cover">
                                    <form method="POST" action="/reservation/{{ $currentReservation->id }}/update" class="m-0 flex justify-center items-center absolute top-0 left-[50%] translate-x-[-100%] md:translate-x-[-50%] h-[250px] md:h-full ">
                                        @csrf
                                        @method('PATCH')
                            ...
                            @if (now()->timezone('Africa/Casablanca')->addHour()->format('Y-m-d\Th:i') < Carbon\Carbon::parse($currentReservation->route->date)->toDateTimeString())
                                <div class="relative flex text-white md:h-[400px] lg:h-64 w-16 items-center justify-between">
                            ...
                            <div class="absolute flex flex-col items-center justify-center bg-white rounded-lg border-2 border-black border-dashed left-[50%] lg:left-[66%] translate-x-[-50%] p-4 lg:space-y-8 top-[50%] translate-y-[-50%]">
                                <img width="100px" src="{{ asset('/images/ship.png') }}" alt="" class="hidden md:block absolute bg-white top-0 left-[50%] translate-x-[-50%] lg:translate-y-[-50%] md:translate-y-[-60%]">
                                <p class="text-sm border-black p-2 rounded mt-8 min-w-32 text-center">{{ $currentReservation->route->driver->user()->first()->name }}</p>
                                <p class="text-sm border-black p-2 rounded min-w-32 text-center block lg:hidden">{{ $currentReservation->route->date }}</p>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        @if($currentReservation == null)
            <div class="pb-8 pr-12 md:mt-12 lg:my-2 md:mx-12 lg:mx-2 md:w-[70%] lg:w-[30%]">
                <div class="bg-white border-black border-4 border-dashed rounded-xl font-black text-xl">
